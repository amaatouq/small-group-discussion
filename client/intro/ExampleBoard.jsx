...
const Arrow = props => {
  const { color, angle, size = 3 } = props;
...
export default class ExampleBoard extends React.Component {
  constructor() {
    super();
    this.state = {
      dots: [],
  ...
  renderGuess() {
    const { stageName, guess } = this.props;
  ...
  renderAlterGuess = () => {
    const { socialGuess } = this.props;
  
    return (
      <Arrow key={socialGuess} size={1.5} angle={socialGuess} color={"#FF4500"} />
    );
  };
  ...
  renderAnswer() {
    ...
    return <Arrow key={"answer"} angle={answer} size={4} />;
  }
    ...
    return (
      <div>
        ...
        >
          {this.renderDots()}
          {this.renderGuess()}
          {socialGuess? this.renderAlterGuess():null}
          {this.renderAnswer()}
        </div>
      </div>
    );
